...
#define ENGINE_USE_MANY_LOCKS(count)	Lock _locks[count];
#define ENGINE_USE_LOCK				USE_MANY_LOCKS(1)
#define	ENGINE_READ_LOCK_IDX(idx)		ReadLockGuard readLockGuard_##idx(_locks[idx], typeid(this).name());
#define ENGINE_READ_LOCK				READ_LOCK_IDX(0)
#define	ENGINE_WRITE_LOCK_IDX(idx)		WriteLockGuard writeLockGuard_##idx(_locks[idx], typeid(this).name());
#define ENGINE_WRITE_LOCK				WRITE_LOCK_IDX(0)

/*---------------
	  Crash
---------------*/

#define ENGINE_CRASH(cause)						\
{											\
...
#define ENGINE_ASSERT_CRASH(expr)			\
{									\
	if (!(expr))					\
	{								\
		CRASH("ASSERT_CRASH");		\
