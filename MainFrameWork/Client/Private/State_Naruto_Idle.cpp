...
void CState_Naruto_Idle::Tick_State_Control(_float fTimeDelta)
{
	...
	if (KEY_TAP(KEY::P))
	{
		CGameInstance::GetInstance()->PlaySound_Distance_LoopChannel(L"Move_2.wav", g_fVolume * 0.5f, m_pPlayer->Get_TransformCom()->Get_State(CTransform::STATE_POSITION), 25.0f);
	}
