...
CSkill_FireBall_Server::CSkill_FireBall_Server(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CSkill_Server(pDevice, pContext)
{
}

CSkill_FireBall_Server::CSkill_FireBall_Server(const CSkill_FireBall_Server& rhs)
	: CSkill_Server(rhs)
{
}

HRESULT CSkill_FireBall_Server::Initialize_Prototype()
{
...
HRESULT CSkill_FireBall_Server::Initialize(void* pArg)
{
...
void CSkill_FireBall_Server::Tick(_float fTimeDelta)
{
	//__super::Tick(fTimeDelta);

}

void CSkill_FireBall_Server::LateTick(_float fTimeDelta)
{
...
HRESULT CSkill_FireBall_Server::Render()
{
	return S_OK;
}

void CSkill_FireBall_Server::OnCollisionEnter(const _uint iColLayer, CCollider* pOther)
{
...
void CSkill_FireBall_Server::OnCollisionStay(const _uint iColLayer, CCollider* pOther)
{
	
}

void CSkill_FireBall_Server::OnCollisionExit(const _uint iColLayer, CCollider* pOther)
{
...
HRESULT CSkill_FireBall_Server::Ready_Coliders()
{
...
void CSkill_FireBall_Server::Send_SkillInfo()
{
...
void CSkill_FireBall_Server::Explosion()
{
	m_bExplosion = true;
...
HRESULT CSkill_FireBall_Server::Ready_Components()
{
...
CSkill_FireBall_Server* CSkill_FireBall_Server::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CSkill_FireBall_Server* pInstance = new CSkill_FireBall_Server(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		cout << "Failed To Created : CSkill_FireBall_Server" << endl;
...
CGameObject* CSkill_FireBall_Server::Clone(void* pArg)
{
	CSkill_FireBall_Server* pInstance = new CSkill_FireBall_Server(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		cout << "Failed To Cloned : CSkill_FireBall_Server" << endl;
		Safe_Release(pInstance);
	}
...
void CSkill_FireBall_Server::Free()
{
	__super::Free();
}
