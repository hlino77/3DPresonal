...
CState_Sasuke_Skill_Chidori_RunStart::CState_Sasuke_Skill_Chidori_RunStart(const wstring& strStateName, CPlayer_Sasuke* pPlayer)
	:CState(strStateName)
	, m_pPlayer(pPlayer)
{
}

CState_Sasuke_Skill_Chidori_RunStart::CState_Sasuke_Skill_Chidori_RunStart(const CState& rhs, CStateMachine* pMachine)
	:CState(rhs, pMachine)
{
}

HRESULT CState_Sasuke_Skill_Chidori_RunStart::Initialize()
{
	m_iAnimIndex = m_pPlayer->Get_ModelCom()->Initailize_FindAnimation(L"Ninjutsu_Chidori_Charge_Lv2ToLv3_ConectToRun", 1.0f);

	if (m_iAnimIndex == -1)
		return E_FAIL;

	if(m_pPlayer->Is_Control())
		m_TickFunc = &CState_Sasuke_Skill_Chidori_RunStart::Tick_State_Control;
	else
		m_TickFunc = &CState_Sasuke_Skill_Chidori_RunStart::Tick_State_NoneControl;


	return S_OK;
}

void CState_Sasuke_Skill_Chidori_RunStart::Enter_State()
{
	Set_NearTarget();
...
void CState_Sasuke_Skill_Chidori_RunStart::Exit_State()
{

}

void CState_Sasuke_Skill_Chidori_RunStart::Tick_State_Control(_float fTimeDelta)
{
	...
	if (pPlayerModel->Is_AnimationEnd(m_iAnimIndex))
	{
		...
		{
			m_pPlayer->Set_State(L"Chidori_RunLoop");
			return;
		}
	}
...
void CState_Sasuke_Skill_Chidori_RunStart::Tick_State_NoneControl(_float fTimeDelta)
{
	Follow_Target(fTimeDelta);
...
void CState_Sasuke_Skill_Chidori_RunStart::Free()
{
	__super::Free();
}

void CState_Sasuke_Skill_Chidori_RunStart::Set_NearTarget()
{
...
void CState_Sasuke_Skill_Chidori_RunStart::Follow_Target(_float fTimeDelta)
{
