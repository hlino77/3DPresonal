...
class CUI_HP_Monster final : public CUI
{


protected:
	/* 원형을 생성할 때 */
	CUI_HP_Monster(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	/* 사본을 생성할 때 */
	CUI_HP_Monster(const CGameObject& rhs); /* 복사 생성자. */

	virtual ~CUI_HP_Monster() = default;
...
private:
	...
	_uint m_iPrevHp;
public:
	static CUI_HP_Monster* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
