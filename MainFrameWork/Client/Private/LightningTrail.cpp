...
CLightningTrail::CLightningTrail(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect(pDevice, pContext)
{
}

CLightningTrail::CLightningTrail(const CLightningTrail& rhs)
	: CEffect(rhs)
{
}

HRESULT CLightningTrail::Initialize_Prototype()
{
	
	return S_OK;
}

HRESULT CLightningTrail::Initialize(void* pArg)
{
	...
	m_szModelName = L"Effect_LightningTrail";
...
void CLightningTrail::Tick(_float fTimeDelta)
{
	...
	if (m_bDisappear)
	{
		...
		if (m_vColor.w <= 0.0f)
		{
			m_vColor.w = 0.0f;
			Set_Active(false);
			CPool<CLightningTrail>::Return_Obj(this);
		}
	}
	else
	{
...
void CLightningTrail::LateTick(_float fTimeDelta)
{
	m_pRendererCom->Add_RenderGroup(CRenderer::RENDER_ALPHABLEND, this);
}

HRESULT CLightningTrail::Render()
{
...
HRESULT CLightningTrail::Ready_Components()
{
...
HRESULT CLightningTrail::Ready_InstanceBuffer()
{
...
void CLightningTrail::Add_Point()
{

	Vec3 vStartPos(*m_vTrailPos);
...
CGameObject* CLightningTrail::Clone(void* pArg)
{
	CLightningTrail* pInstance = new CLightningTrail(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CLightningTrail");
...
CLightningTrail* CLightningTrail::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLightningTrail* pInstance = new CLightningTrail(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CLightningTrail");
		Safe_Release(pInstance);
	}
