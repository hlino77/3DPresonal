...
CLightning_A_01::CLightning_A_01(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect(pDevice, pContext)
{
}

CLightning_A_01::CLightning_A_01(const CLightning_A_01& rhs)
	: CEffect(rhs)
{
}

HRESULT CLightning_A_01::Initialize_Prototype()
{
	
	return S_OK;
}

HRESULT CLightning_A_01::Initialize(void* pArg)
{
	__super::Initialize(pArg);


	m_szModelName = L"Effect_Lightning_A_01";
...
void CLightning_A_01::Tick(_float fTimeDelta)
{
	...
	{
		Set_Active(false);
		CPool<CLightning_A_01>::Return_Obj(this);
	}
}

void CLightning_A_01::LateTick(_float fTimeDelta)
{
	m_pRendererCom->Add_RenderGroup(CRenderer::RENDER_MODELEFFECT_INSTANCE, this);
}

HRESULT CLightning_A_01::Render()
{
	
	return S_OK;
}

HRESULT CLightning_A_01::Render_Instance(ID3D11Buffer* pInstanceBuffer, _uint iSize)
{
...
void CLightning_A_01::Add_InstanceData(vector<Vec4>& BufferData)
{
...
void CLightning_A_01::Appear(Vec3 vPos)
{
...
void CLightning_A_01::DisAppear()
{
	m_bEnd = true;
}
...
HRESULT CLightning_A_01::Ready_Components()
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	...
	///* For.Com_Model */
	wstring strComName = L"Prototype_Component_Model_Lightning_A_01";
...
CGameObject* CLightning_A_01::Clone(void* pArg)
{
	CLightning_A_01* pInstance = new CLightning_A_01(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CLightning_A_01");
...
CLightning_A_01* CLightning_A_01::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLightning_A_01* pInstance = new CLightning_A_01(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CLightning_A_01");
		Safe_Release(pInstance);
	}
