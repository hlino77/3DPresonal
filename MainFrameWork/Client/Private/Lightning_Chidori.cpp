...
CLightning_Chidori::CLightning_Chidori(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect(pDevice, pContext)
{
}

CLightning_Chidori::CLightning_Chidori(const CLightning_Chidori& rhs)
	: CEffect(rhs)
{
}

HRESULT CLightning_Chidori::Initialize_Prototype()
{

	return S_OK;
}

HRESULT CLightning_Chidori::Initialize(void* pArg)
{
	__super::Initialize(pArg);


	m_szModelName = L"Effect_Lightning_Chidori";
...
void CLightning_Chidori::Tick(_float fTimeDelta)
{
	...
	{
		Set_Active(false);
		CPool<CLightning_Chidori>::Return_Obj(this);
	}
}

void CLightning_Chidori::LateTick(_float fTimeDelta)
{
	m_pRendererCom->Add_RenderGroup(CRenderer::RENDER_MODELEFFECT_INSTANCE, this);
}

HRESULT CLightning_Chidori::Render()
{

	return S_OK;
}

HRESULT CLightning_Chidori::Render_Instance(ID3D11Buffer* pInstanceBuffer, _uint iSize)
{
...
void CLightning_Chidori::Add_InstanceData(vector<Vec4>& BufferData)
{
...
void CLightning_Chidori::Appear(Vec3 vPos)
{
...
void CLightning_Chidori::DisAppear()
{
	m_bEnd = true;
}
...
HRESULT CLightning_Chidori::Ready_Components()
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	...
	///* For.Com_Model */
	wstring strComName = L"Prototype_Component_Model_Lightning_Chidori";
...
CGameObject* CLightning_Chidori::Clone(void* pArg)
{
	CLightning_Chidori* pInstance = new CLightning_Chidori(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CLightning_Chidori");
...
CLightning_Chidori* CLightning_Chidori::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLightning_Chidori* pInstance = new CLightning_Chidori(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CLightning_Chidori");
		Safe_Release(pInstance);
	}
